  ...
  return (
    <div className="bg-nb-s0 rounded-2xl border border-nb-border shadow-nb overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        {/* 股票搜尋 */}
        <div>
          <label className="text-xs text-nb-t2 mb-1 block">股票</label>
          <input
            value={query}
            onChange={e => doSearch(e.target.value)}
            placeholder="輸入代號或名稱，例如 2330、台積電"
            className="w-full h-11 px-3.5 bg-nb-s4 border border-nb-border2 rounded-xl text-sm focus:outline-none focus:border-nb-orange/50"
          />
          {sugg.length > 0 && (
            <div className="mt-1 border border-nb-border rounded-xl overflow-hidden">
              {sugg.map(s => (
                <button
                  key={s.code}
                  onClick={() => pick(s)}
                  className="w-full flex justify-between items-center px-3.5 py-2.5 hover:bg-nb-s4 text-sm"
                >
          ...
          {selected && (
            <div className="mt-1.5 text-xs text-nb-down font-medium">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-xs text-nb-t2 mb-1 block">發放日期</label>
            <input
              type="date"
              value={date}
          ...
          <div>
            <label className="text-xs text-nb-t2 mb-1 block">金額（元）</label>
            <input
              type="number"
              inputMode="decimal"
              value={amount}
        ...
        <button
          onClick={handleSave}
          disabled={!valid}
          className="w-full h-12 bg-nb-orange hover:hover:bg-nb-orange/90 disabled:bg-nb-border disabled:text-nb-t2 text-white text-sm font-extrabold rounded-xl shadow-nb transition-colors"
        >
          儲存股息紀錄
        </button>
      </div>
          ...
          <div className="bg-gradient-to-br from-nb-t0 to-[#1A1510] rounded-2xl p-4 shadow-nb-md">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center">
                <div className="text-[9px] text-nb-t2 mb-1 font-medium">累積股息</div>
                <div className="text-base font-extrabold text-nb-down leading-tight">
                  ${fmt(overview.totalIncome)}
                </div>
              </div>
              <div className="text-center">
                <div className="text-[9px] text-nb-t2 mb-1 font-medium">今年股息</div>
                <div className="text-base font-extrabold text-nb-down leading-tight">
                  ${fmt(overview.thisYearIncome)}
                </div>
              </div>
              <div className="text-center">
                <div className="text-[9px] text-nb-t2 mb-1 font-medium">紀錄筆數</div>
              ...
              {overview.byStock.map(s => (
                <div key={s.code}>
                  <button
                    ...
                    className="w-full flex justify-between items-center px-4 py-3 hover:bg-nb-s4 transition-colors"
                  >
                    <div className="text-left">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-nb-t0">{s.name}</span>
                        <span className="text-[10px] text-nb-t2 bg-nb-s4 px-2 py-0.5 rounded-full">{s.code}</span>
                      </div>
                      <div className="text-[10px] text-nb-t2 mt-0.5">
                        {s.count} 筆・最近 {s.latestDate}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-extrabold text-nb-down">+${fmt(s.totalAmount)}</span>
                      <span className={`text-nb-t3 text-xs transition-transform ${expandedStock === s.code ? 'rotate-180' : ''}`}>▼</span>
                    </div>
                  </button>

                  {/* 展開：該股票的所有股息明細 */}
                  {expandedStock === s.code && (
                    <div className="px-4 pb-3 bg-nb-s4/50">
                      {dividends
                        .filter(d => d.code === s.code)
                        .sort((a, b) => b.date.localeCompare(a.date))
                        .map(d => (
                          <div key={d.id} className="flex justify-between items-center py-2 border-b border-nb-border last:border-0">
                            <div>
                              <div className="text-xs text-nb-t1 font-medium">{d.date}</div>
                              {d.note && <div className="text-[10px] text-nb-t2 mt-0.5">{d.note}</div>}
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-bold text-nb-down">+${fmt(d.amount)}</span>
                              <button
                                onClick={() => deleteDividend(d.id)}
                                className="w-5 h-5 rounded-full bg-nb-s0 hover:bg-red-100 text-nb-t3 hover:text-nb-up text-xs flex items-center justify-center transition-colors"
                              >×</button>
                            </div>
                          </div>
                        ))}
              ...
              {overview.byYear.map(y => (
                <div key={y.year} className="flex justify-between items-center px-4 py-3">
                  <div>
                    ...
                    <span className="text-[10px] text-nb-t2 ml-2">{y.count} 筆</span>
                  </div>
                  <span className="text-sm font-extrabold text-nb-down">+${fmt(y.totalAmount)}</span>
                </div>
              ))}
            </div>
          </div>
